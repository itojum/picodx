...
Window.loop do
  ...
  held << "A"      if Input.key_down?(K_A)
  held << "Z"      if Input.key_down?(K_Z)

  [K_LEFT, K_RIGHT, K_UP, K_DOWN, K_SPACE, K_ESCAPE, K_RETURN, K_A, K_Z].each do |k|
    if Input.key_push?(k)
      last_pushed = k
      push_flash  = 8
    end
  ...
  # key_push? indicator: lights up only on the first frame a key is pressed
  if push_flash > 0
